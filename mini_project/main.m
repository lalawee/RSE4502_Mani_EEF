...
% Search over joint angle combinations
for q1 = pi          % base yaw fixed at pi
    for q2 = linspace(-pi/2, pi/2, 20)
        for q3 = linspace(-pi, 0, 20)
            for q4 = linspace(-pi/2, pi/2, 10)
                for q5 = 0
                    q_try = [q1; q2; q3; q4; q5];
...
fprintf('Refining...\n');
bq = best_q;
for q2 = linspace(bq(2)-0.2, bq(2)+0.2, 30)
    for q3 = linspace(bq(3)-0.2, bq(3)+0.2, 30)
        for q4 = linspace(bq(4)-0.2, bq(4)+0.2, 30)
            q_try = [pi; q2; q3; q4; 0];
...
grid on;
visualize(robot, path, q_hist, qdot_hist, ee_hist, w_hist, lambda_hist);